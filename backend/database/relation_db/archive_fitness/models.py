...
from sqlalchemy import Column, Integer, String, DateTime, Date, ForeignKey, Float, Text, Boolean
...
class MealLog(Base):
    ...
    meal_type = Column(String(20))  # breakfast, lunch, dinner, snack, pre_workout, post_workout
    ...
    meal_photo_url = Column(String(500))
    notes = Column(Text)
...
class NutritionGoal(Base):
    """영양 목표 테이블 (Nutrition Agent)"""
    __tablename__ = "nutrition_goals"

    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    goal_type = Column(String(50))  # weight_loss, muscle_gain, maintenance, health
    target_calories = Column(Integer)
    target_protein = Column(Float)  # grams
    target_carbs = Column(Float)  # grams
    target_fat = Column(Float)  # grams
    target_water = Column(Integer)  # ml
    start_date = Column(DateTime, nullable=False)
...
class FoodDatabase(Base):
    """음식 영양 정보 DB (Nutrition Agent)"""
    __tablename__ = "food_database"

    id = Column(Integer, primary_key=True, autoincrement=True)
    name = Column(String(200), nullable=False)
    name_en = Column(String(200))
    category = Column(String(50))  # protein, carbs, vegetables, fruits, dairy, snacks, beverages
    serving_size = Column(Float)  # 1회 제공량
    serving_unit = Column(String(20))  # g, ml, 개, 공기
    ...
    source = Column(String(50))  # user_input, korean_fdc, usda
    created_at = Column(DateTime, default=datetime.utcnow)


class DailyNutritionSummary(Base):
    """일별 영양 요약 테이블 (Nutrition Agent)"""
    ...
    goal_achievement_rate = Column(Float)  # 0.0 ~ 1.0
    quality_score = Column(Float)  # 0.0 ~ 1.0
...
class NutritionFeedback(Base):
    ...
    feedback_type = Column(String(50))  # daily_summary, meal_specific, weekly_review
    feedback_text = Column(Text, nullable=False)
    recommendations = Column(Text)  # JSON array
    created_by = Column(String(100))  # AI_Agent, Trainer_Name
    sentiment = Column(String(20))  # positive, neutral, constructive
    created_at = Column(DateTime, default=datetime.utcnow)
